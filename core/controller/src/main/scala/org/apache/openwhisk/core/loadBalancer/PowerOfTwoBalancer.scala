...
class PowerOfTwoBalancer(
  ...
  override def publish(action: ExecutableWhiskActionMetaData, msg: ActivationMessage)(
    ...
    val chosen = if (invokersToUse.nonEmpty) {
      val hash = PowerOfTwoBalancer.generateHash(msg.user.namespace.name, action.fullyQualifiedName(false))
      val homeInvoker = hash % invokersToUse.size
      val stepSize = stepSizes(hash % stepSizes.size)
      val invoker: Option[(InvokerInstanceId, Boolean)] = PowerOfTwoBalancer.schedule(
...
case class PowerOfTwoBalancerState(
  private var _consistentHash: ConsistentHash[SimpleNode] = HashRing.newBuilder().build(),
  ...
  private var _managedStepSizes: Seq[Int] = PowerOfTwoBalancer.pairwiseCoprimeNumbersUntil(0),
  private var _blackboxStepSizes: Seq[Int] = PowerOfTwoBalancer.pairwiseCoprimeNumbersUntil(0),
  ...
  def updateInvokers(newInvokers: IndexedSeq[InvokerHealth]): Unit = {
    ...
    val logDetail = if (oldSize != newSize) {
      _managedStepSizes = PowerOfTwoBalancer.pairwiseCoprimeNumbersUntil(managed)
      _blackboxStepSizes = PowerOfTwoBalancer.pairwiseCoprimeNumbersUntil(blackboxes)
